MethodDka メニュー
因数分解専用版（整数丸め版）
高精度版（50桁 DKA 法）
README
MethodDka の 5 種類のバージョン
MethodDka は、用途に応じて以下の 5 種類の形態で提供しています。

1. 因数分解専用版（HTML / オフライン動作）
ダウンロードして使用できます
オフライン動作可能
インストール不要（HTML を開くだけ）
因数分解専用版（HTML）をダウンロード
2. 高精度版（HTML / 50桁 / オフライン動作）
ダウンロードして使用できます
オフライン動作可能
インストール不要
高精度版（HTML）をダウンロード
3. GitHub Pages 版：因数分解専用版（オンライン専用）
ブラウザで即動作
ダウンロード不要
因数分解専用版（オンライン）
4. GitHub Pages 版：高精度版（オンライン専用）
ブラウザで即動作
ダウンロード不要
高精度版（オンライン）
5. Windows 11 高精度版（インストーラ版 / オフライン動作）
Windows 11 Home 25H2 対応
インストールして使用
オフライン動作可能
MSI インストーラ（Windows 11）
APPX パッケージ（Microsoft Store 互換）
MethodDka
因数分解したり、高次代数方程式を解くプログラム
高精度バージョン（50桁）

概要
MethodDka.html（JavaScript）
対応OS：Windows 11 Home 25H2、Android 15
MIT ライセンス

作者：小泉 嘉章
メールアドレス：ja142397@s6.dion.ne.jp
試作評価版バージョン：6.00

数学的背景
1次方程式は中学1年生、2次方程式は中学3年生で解を学びます。
3次方程式以上はカルダノの公式や数値解析（ラグランジュ法・ニュートン法）で求められますが、
4次以上の一般方程式は数値解析によらなければ解けません。

MethodDka の特徴
実数係数の高次（最大1000次）多項式に対応
複素数を含む全ての解を求められる
初期値なしで収束（DKA法：Durand–Kerner–Aberth 法）
ブラウザだけで動作（Edge / Chrome）
スマホ（Android）でも動作
使い方
最高次数を入力する
表示された入力ボックスに、次数の高い順から係数を入力
次数が存在しない場合は必ず「0」を入力（省略不可）
係数が +1 の場合は「1」、-1 の場合は「-1」を入力
計算開始ボタンを押すと、全ての解が表示される
解の有効数字は約50桁です。
注意事項
本プログラムを使用して発生した問題について、作者は一切責任を負いません。
必ず自己責任で使用し、必要に応じて検算してください。

作者からのメッセージ
最後までヘルプファイルをお読みいただきありがとうございます。
このプログラムが皆様のお役に立てれば幸いです。

完全日本語版にバージョンアップしました。
算数・数学を教えてくださった恩師、数値解析の書籍の著者の方々に深く感謝いたします。

ライセンス
試作評価版のためフリーソフトとします。

技術的背景
このプログラムは、Microsoft Edge の Windows Copilot により、Android 用 Java ソースコードを JavaScript に変換して作成されました。
さらに、50桁の高精度バージョンも Windows Copilot により生成されました。
Microsoft 社様に感謝いたします。

DKA 法が収束しない場合の要因
初期値の選定が不適切
重根や近接する根が存在する
係数のスケールが極端に異なる
反復回数や許容誤差の設定が不十分
複素数演算での丸め誤差の蓄積
改善のヒント
